...
export default function Contact() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://127.0.0.1:8000/api/v1'}/profile/`)
      ...
      .catch(err => console.error("Error loading profile in Contact:", err));
  }, []);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://127.0.0.1:8000/api/v1'}/contact/`, {
        method: "POST",
        headers: {
  ...
  const socials = [
    ...
    },
    {
      ...
      href: profile?.upwork_url || "https://upwork.com",
      icon: (
        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      href: profile?.email ? `mailto:${profile.email}` : "mailto:salman@example.com",
      icon: (
        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
